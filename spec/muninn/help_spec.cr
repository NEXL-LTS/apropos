...
private def run_help(args : Array(String)) : {Int32, String}
  ...
  code = Muninn::Help.run(args, stdout)
  {code, stdout.to_s}
end
...
private def usage_command_names : Set(String)
  ...
  capturing = false
  Muninn::CLI::USAGE.each_line do |line|
...
describe Muninn::Help do
  ...
  describe ".run (per command)" do
    it "prints the mental-model note for a known command and defers to --help" do
      ...
      code.should eq(0)
      stdout.should contain("muninn review —")
      stdout.should contain("Run `muninn review --help` for exact flags.")
    end
