...
describe Muninn::Index do
  describe ".build" do
    it "captures classification, triggers, and skill metadata per doc" do
      ...
      index = Muninn::Index.build(conventions)

      index.schema_version.should eq(Muninn::Index::SCHEMA_VERSION)
  ...
  describe "serialization" do
    it "round-trips through the deterministic document form" do
      ...
      document = Muninn::Index.build(conventions).to_document

      document.ends_with?("\n").should be_true
      document.should contain("\"schema_version\": 1")

      loaded = Muninn::Index.load(document)
      loaded.should_not be_nil
      loaded.as(Muninn::Index).docs.first.path.should eq("docs/conventions/a.md")
    end

    it "is byte-stable across builds" do
      conventions = [convention("docs/conventions/a.md", %(paths: ["src/**"]))]
      Muninn::Index.build(conventions).to_document
        .should eq(Muninn::Index.build(conventions).to_document)
    end
  end

  describe ".load" do
    it "returns nil for malformed JSON" do
      Muninn::Index.load("{not json").should be_nil
    end

    it "returns nil on a schema-version mismatch (forces rebuild)" do
      stale = %({"schema_version": 999, "docs": []})
      Muninn::Index.load(stale).should be_nil
    end
  end
  ...
  describe "#covers?" do
    it "is true when every doc path and hash is unchanged" do
      conventions = [convention("docs/conventions/a.md", %(paths: ["src/**"]))]
      Muninn::Index.build(conventions).covers?(conventions).should be_true
    end
    ...
    it "is false when a doc's content hash changed" do
      ...
      Muninn::Index.build(original).covers?(edited).should be_false
    end
    ...
    it "is false when a doc was added or removed" do
      ...
      Muninn::Index.build(one).covers?(two).should be_false
    end
  end
end
