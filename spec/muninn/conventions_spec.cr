...
private class FakeFS < Muninn::Filesystem
...
private def convention(frontmatter : String) : Muninn::Convention
  Muninn::Convention.parse("docs/conventions/rule.md", "---\n#{frontmatter}\n---\nbody\n")
end

describe Muninn::Convention do
  describe ".parse" do
    it "hashes the whole doc text" do
      text = "---\npaths: [\"src/**\"]\n---\nbody\n"
      Muninn::Convention.parse("docs/conventions/a.md", text).hash
        .should eq(Digest::SHA256.hexdigest(text))
    end

    it "keeps a doc with no frontmatter as reference-only with the full body" do
      conv = Muninn::Convention.parse("docs/conventions/a.md", "# Just prose\n")
...
describe Muninn::Conventions do
  describe ".walk" do
    it "walks docs sorted, with repo-relative POSIX paths (in-memory)" do
      ...
      conventions = Muninn::Conventions.walk(root, fs)
    ...
    it "reads real files through the default adapter" do
      ...
      begin
        ...
        conventions = Muninn::Conventions.walk(Path[dir])
        conventions.map(&.path).should eq([
          "docs/conventions/a.md",
          "docs/conventions/workflows/b.md",
        ])
