...
private def run_lint(fs : Muninn::Filesystem, strict : Bool = false) : {Int32, String}
  stdout = IO::Memory.new
  stderr = IO::Memory.new
  code = Muninn::Lint.run(ROOT, fs, strict, stdout, stderr)
  {code, stdout.to_s}
end
...
private def wrapper_for(name : String, text : String) : {String, String}
  convention = Muninn::Convention.parse("docs/conventions/#{name}", text)
  slug, content = Muninn::Skills.wrappers([convention]).first
  {"/repo/.claude/skills/#{slug}/SKILL.md", content}
end

describe Muninn::Lint do
  ...
  it "warns on a skill doc over the line budget" do
    body = String.build { |io| (Muninn::Lint::SKILL_DOC_MAX + 1).times { io << "line\n" } }
  ...
  it "warns when a root file exceeds its line budget but not when it is short" do
    big = String.build { |io| (Muninn::Lint::ROOT_FILE_MAX + 1).times { io << "x\n" } }
    fs = InMemoryFS.new({
      "/repo/AGENTS.md" => big,
