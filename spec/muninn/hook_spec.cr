...
private class ExplodingFS < Muninn::Filesystem
...
private def invoke(event : Symbol, input : String, fs : Muninn::Filesystem,
                   override : String? = "/repo", now : Time = NOW, verbose : Bool = false) : {Int32, String}
  stdout = IO::Memory.new
  reader = IO::Memory.new(input)
  code =
    if event == :pre
      Muninn::Hook.pre(reader, stdout, fs, now, override, verbose)
    else
      Muninn::Hook.post(reader, stdout, fs, now, override, verbose)
    end
  {code, stdout.to_s}
end

describe Muninn::Hook do
  describe ".pre" do
    it "injects a matching Layer 2 rule before the edit" do
      ...
      fs.files.has_key?("/repo/.cache/muninn/index.json").should be_true
      fs.files.has_key?("/repo/.cache/muninn/sessions/s.json").should be_true
    end
    ...
    it "emits nothing when no repo root can be resolved" do
      input = pre_json("src/app.cr", cwd: File.tempname("muninn-norepo"))
    ...
    it "uses an existing index and emits nothing when the source doc is unreadable" do
      index = Muninn::Index.build([Muninn::Convention.parse("docs/conventions/a.md", A_DOC)])
      fs = InMemoryFS.new({"/repo/.cache/muninn/index.json" => index.to_document})
    ...
    it "still injects when the cache is unwritable and dedup is unavailable" do
      ...
      code.should eq(0)
      stdout.should contain("Convention (docs/conventions/a.md):")
      fs.files.has_key?("/repo/.cache/muninn/index.json").should be_false
    end
    ...
    it "logs the failure under the override root when verbose" do
      ...
      code.should eq(0)
      fs.appended.map(&.first).should eq(["/repo/.cache/muninn/log"])
    end
    ...
    it "logs to the process directory when verbose with no override" do
      ...
      fs.appended.first.first.should eq(File.join(Dir.current, ".cache", "muninn", "log"))
    end
