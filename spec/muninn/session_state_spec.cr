...
private def session_path(id : String) : String
  "/repo/.cache/muninn/sessions/#{id}.json"
end
...
private class PhantomFS < Muninn::Filesystem
...
describe Muninn::SessionState do
  describe ".load" do
    it "is empty when there is no session id" do
      Muninn::SessionState.load(ROOT, InMemoryFS.new, nil).injected.should be_empty
    end

    it "is empty when the session file is absent" do
      Muninn::SessionState.load(ROOT, InMemoryFS.new, "s").injected.should be_empty
    end

    it "reads back a persisted injected set" do
      fs = InMemoryFS.new
      state = Muninn::SessionState.new
      state.add("docs/conventions/a.md")
      state.save(ROOT, fs, "s", NOW)

      loaded = Muninn::SessionState.load(ROOT, fs, "s")
    ...
    it "treats a corrupt session file as empty (fail open)" do
      ...
      Muninn::SessionState.load(ROOT, fs, "s").injected.should be_empty
    end
  end
  ...
  describe "#save" do
    ...
    it "is a no-op without a session id" do
      fs = InMemoryFS.new
      Muninn::SessionState.new.save(ROOT, fs, nil, NOW)
  ...
  describe ".prune" do
    it "removes session files older than the max age and keeps fresh ones" do
      old = Muninn::SessionState::Document.new((NOW - 8.days).to_unix, ["a.md"])
      fresh = Muninn::SessionState::Document.new((NOW - 1.day).to_unix, ["b.md"])
      fs = InMemoryFS.new({
        session_path("old")   => old.to_json,
        session_path("fresh") => fresh.to_json,
      })

      Muninn::SessionState.prune(ROOT, fs, NOW)
    ...
    it "skips a session file that cannot be read (listing race)" do
      Muninn::SessionState.prune(ROOT, PhantomFS.new([session_path("gone")]), NOW)
    end
