...
private class FakeGit < Muninn::Git
  ...
  def diff(repo_root : Path, range : String) : String
    @diffed_range = range
    raise Muninn::Git::Error.new("diff boom") if @diff_raises
    @diff_text
  end
...
private def run_match(fs : Muninn::Filesystem, paths : Array(String),
                      format : String = "paths", stdin_content : String? = nil) : {Int32, String, String}
  stdout = IO::Memory.new
  stderr = IO::Memory.new
  code = Muninn::Review.match(ROOT, fs, paths, format, stdin_content, stdout, stderr)
  {code, stdout.to_s, stderr.to_s}
end

private def run_review(fs : Muninn::Filesystem, git : Muninn::Git,
                       range : String? = nil, format : String = "md") : {Int32, String, String}
  stdout = IO::Memory.new
  stderr = IO::Memory.new
  code = Muninn::Review.run(ROOT, fs, git, range, format, stdout, stderr)
  {code, stdout.to_s, stderr.to_s}
end
...
describe Muninn::Review do
  describe ".match" do
    ...
    it "fails closed on a malformed convention doc" do
      ...
      code.should eq(1)
      stderr.should contain("muninn match:")
    end
  end
  ...
  describe ".run" do
    ...
    it "fails closed when git fails" do
      ...
      code.should eq(1)
      stderr.should contain("muninn review: diff boom")
    end
  end
end
