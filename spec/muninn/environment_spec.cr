...
describe Muninn::Environment::Real do
  env = Muninn::Environment::Real.new

  it "resolves an executable on PATH and returns nil for a missing one" do
    env.which("sh").should_not be_nil
    env.which("muninn-definitely-absent-xyz").should be_nil
  end
  ...
  it "returns nil when the command cannot be launched" do
    env.run_capture("muninn-definitely-absent-xyz", [] of String).should be_nil
  end
end
