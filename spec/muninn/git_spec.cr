...
describe Muninn::Git::Real do
  it "produces a unified diff for a range" do
    with_repo do |dir|
      diff = Muninn::Git::Real.new.diff(Path[dir], "main...feature")
  ...
  it "returns nil for an absent symbolic ref (no remote configured)" do
    with_repo do |dir|
      Muninn::Git::Real.new.symbolic_ref(Path[dir], "refs/remotes/origin/HEAD").should be_nil
    end
  end

  it "raises a Git::Error when the git command fails" do
    with_repo do |dir|
      expect_raises(Muninn::Git::Error, "git diff") do
        Muninn::Git::Real.new.diff(Path[dir], "no-such-ref...HEAD")
      end
    end
  end
end
