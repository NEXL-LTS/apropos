...
private def run_muninn(binary : String, args : Array(String), stdin : String = "") : {Int32, String}
...
describe "muninn review/match (binary)" do
  binary = File.join(Dir.tempdir, "muninn-review-#{Process.pid}")
  ...
  it "matches a path and reviews a git range end to end" do
    dir = File.tempname("muninn-review-repo")
    begin
      ...
      code, matched = run_muninn(binary, ["match", "--repo-root", dir, "app/models/user.cr"])
      code.should eq(0)
      matched.should eq("docs/conventions/models.md\n")

      code, manifest = run_muninn(binary, ["review", "--repo-root", dir, "main...HEAD"])
      code.should eq(0)
