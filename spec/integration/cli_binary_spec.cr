...
# End-to-end coverage of the compiled entry point (src/muninn.cr), which unit
...
describe "muninn binary" do
  binary = File.join(Dir.tempdir, "muninn-int-#{Process.pid}")
  ...
  it "prints the version and exits 0" do
    ...
    stdout.to_s.should contain("muninn #{Muninn::VERSION}")
  end
  ...
  it "prints usage with no args and exits 0" do
    ...
    status.exit_code.should eq(0)
    stdout.to_s.should contain("Usage: muninn")
  end
