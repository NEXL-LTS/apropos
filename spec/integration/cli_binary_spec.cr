...
# End-to-end coverage of the compiled entry point (src/agent_apropos.cr), which unit
...
describe "agent-apropos binary" do
  binary = File.join(Dir.tempdir, "agent-apropos-int-#{Process.pid}")
  ...
  it "prints the version and exits 0" do
    ...
    stdout.to_s.should contain("agent-apropos #{AgentApropos::VERSION}")
  end
  ...
  it "prints usage with no args and exits 0" do
    ...
    status.exit_code.should eq(0)
    stdout.to_s.should contain("Usage: agent-apropos")
  end
