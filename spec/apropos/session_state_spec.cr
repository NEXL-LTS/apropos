...
private class PhantomFS < Apropos::Filesystem
...
describe Apropos::SessionState do
  describe ".load" do
    it "is empty when there is no session id" do
      Apropos::SessionState.load(ROOT, InMemoryFS.new, nil).injected.should be_empty
    end

    it "is empty when the session file is absent" do
      Apropos::SessionState.load(ROOT, InMemoryFS.new, "s").injected.should be_empty
    end
    ...
    it "reads back a persisted injected set" do
      ...
      loaded = Apropos::SessionState.load(ROOT, fs, "s")
  ...
  describe "#save" do
    ...
    it "is a no-op without a session id" do
      fs = InMemoryFS.new
      Apropos::SessionState.new.save(ROOT, fs, nil, NOW)
  ...
  describe ".prune" do
    it "removes session files older than the max age and keeps fresh ones" do
      old = Apropos::SessionState::Document.new((NOW - 8.days).to_unix, ["a.md"])
      fresh = Apropos::SessionState::Document.new((NOW - 1.day).to_unix, ["b.md"])
      fs = InMemoryFS.new({
        session_path("old")   => old.to_json,
        session_path("fresh") => fresh.to_json,
      })

      Apropos::SessionState.prune(ROOT, fs, NOW)
    ...
    it "skips a session file that cannot be read (listing race)" do
      Apropos::SessionState.prune(ROOT, PhantomFS.new([session_path("gone")]), NOW)
    end
