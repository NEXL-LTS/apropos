...
describe "Apropos.find_repo_root" do
  ...
  it "walks up from a nested directory to the repo root" do
    ...
    begin
      ...
      Dir.mkdir_p(nested)
      Apropos.find_repo_root(Path[nested]).should eq(Path[dir].expand)
    ensure
      FileUtils.rm_rf(dir)
    end
  end
  ...
  it "returns nil when no .git exists up to the filesystem root" do
    ...
    begin
      Dir.mkdir_p(dir)
      Apropos.find_repo_root(Path[dir]).should be_nil
    ensure
      FileUtils.rm_rf(dir)
    end
  end
