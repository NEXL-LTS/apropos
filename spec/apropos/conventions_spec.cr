...
private class FakeFS < Apropos::Filesystem
...
private def convention(frontmatter : String) : Apropos::Convention
  Apropos::Convention.parse("docs/conventions/rule.md", "---\n#{frontmatter}\n---\nbody\n")
end

describe Apropos::Convention do
  describe ".parse" do
    it "hashes the whole doc text" do
      text = "---\npaths: [\"src/**\"]\n---\nbody\n"
      Apropos::Convention.parse("docs/conventions/a.md", text).hash
        .should eq(Digest::SHA256.hexdigest(text))
    end

    it "keeps a doc with no frontmatter as reference-only with the full body" do
      conv = Apropos::Convention.parse("docs/conventions/a.md", "# Just prose\n")
  ...
  describe "#verify" do
    it "harvests the section under a `## Verify` heading up to the next heading" do
      text = "---\npaths: [\"src/**\"]\n---\n# Rule\n\nBody.\n\n## Verify\n\n- one\n- two\n\n## Notes\n\nignored\n"
      Apropos::Convention.parse("docs/conventions/a.md", text).verify.should eq("- one\n- two")
    end

    it "harvests to end of doc when no heading follows" do
      text = "---\npaths: [\"src/**\"]\n---\nBody.\n\n## Verify\n\nCheck it works.\n"
      Apropos::Convention.parse("docs/conventions/a.md", text).verify.should eq("Check it works.")
    end
    ...
    it "is nil when the Verify section is empty" do
      text = "---\npaths: [\"src/**\"]\n---\nBody.\n\n## Verify\n\n## Next\n\nx\n"
      Apropos::Convention.parse("docs/conventions/a.md", text).verify.should be_nil
    end
  end
...
describe Apropos::Conventions do
  describe ".walk" do
    it "walks docs sorted, with repo-relative POSIX paths (in-memory)" do
      ...
      conventions = Apropos::Conventions.walk(root, fs)
    ...
    it "reads real files through the default adapter" do
      ...
      begin
        ...
        conventions = Apropos::Conventions.walk(Path[dir])
        conventions.map(&.path).should eq([
          "docs/conventions/a.md",
          "docs/conventions/workflows/b.md",
        ])
