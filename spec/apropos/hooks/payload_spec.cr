...
private def parse(json : String) : Apropos::Hook::Payload
  Apropos::Hook::Payload.parse(json) || raise "expected #{json.inspect} to parse"
end

describe Apropos::Hook::Payload do
  describe ".parse" do
    ...
    it "returns nil for malformed JSON (fail open)" do
      Apropos::Hook::Payload.parse("{not json").should be_nil
    end
