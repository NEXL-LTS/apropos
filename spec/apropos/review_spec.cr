...
private class FakeGit < Apropos::Git
  ...
  def diff(repo_root : Path, range : String) : String
    @diffed_range = range
    raise Apropos::Git::Error.new("diff boom") if @diff_raises
    @diff_text
  end
...
private def run_match(fs : Apropos::Filesystem, paths : Array(String),
                      format : String = "paths", stdin_content : String? = nil) : {Int32, String, String}
  stdout = IO::Memory.new
  stderr = IO::Memory.new
  code = Apropos::Review.match(ROOT, fs, paths, format, stdin_content, stdout, stderr)
  {code, stdout.to_s, stderr.to_s}
end

private def run_review(fs : Apropos::Filesystem, git : Apropos::Git,
                       range : String? = nil, format : String = "md") : {Int32, String, String}
  stdout = IO::Memory.new
  stderr = IO::Memory.new
  code = Apropos::Review.run(ROOT, fs, git, range, format, stdout, stderr)
  {code, stdout.to_s, stderr.to_s}
end
