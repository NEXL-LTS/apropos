  ...
  module Lint
    ...
    private def wrapper_findings(repo_root : Path, fs : Filesystem,
      ...
      wrappers.each do |slug, content|
        ...
        if actual.nil?
          findings << Finding.new(:error, wrapper_display(slug), "missing generated wrapper (run `muninn generate`)")
        elsif actual != content
          findings << Finding.new(:error, wrapper_display(slug), "stale generated wrapper (run `muninn generate`)")
        end
      end

      (existing_slugs(repo_root, fs) - wrappers.keys).sort.each do |slug|
        findings << Finding.new(:error, wrapper_display(slug), "orphaned generated wrapper (run `muninn generate`)")
      end
      findings
    end
