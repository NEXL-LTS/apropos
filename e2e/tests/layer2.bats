...
export SENTINEL_L2="apropos-rule:L2-7Q2X"
...
@test "Layer 2 delivery (Claude): hook pre injects the src rule + sentinel for a src/** edit" {
  new_sample with
  run apropos hook pre <<< "$(pre_payload src/util.py)"
  assert_success
...
@test "Layer 2 with apropos (Claude): src edit carries the sentinel" {
  require_live_claude
  new_sample with
...
@test "Layer 2 without apropos (Claude): the sentinel does not appear" {
  require_live_claude
  new_sample without
  run_claude "$PROMPT_L2"
...
@test "Layer 2 delivery (OpenCode): plugin payload format produces L2 context via apropos hook pre" {
  # Simulate what the plugin does: call apropos hook pre with the payload format
  ...
  run apropos hook pre --repo-root "$WORK" <<< "$payload"
  assert_success
  assert_output --partial '"hookEventName":"PreToolUse"'
  assert_output --partial "$SENTINEL_L2"
  assert_output --partial 'docs/conventions/src-rule.md'
