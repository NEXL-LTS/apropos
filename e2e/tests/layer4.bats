...
export SENTINEL_L4="apropos-rule:L4-Q7X2"
...
@test "Layer 4 with apropos (Claude): the new arithmetic op carries the sentinel" {
  require_live_claude
  new_sample with
...
@test "Layer 4 without apropos (Claude): the sentinel does not appear" {
  require_live_claude
  new_sample without
  run_claude "$PROMPT_L4"
...
@test "Layer 4 with apropos (OpenCode): the new arithmetic op carries the sentinel" {
...
@test "Layer 4 without apropos (OpenCode): the sentinel does not appear" {
  require_live_opencode
  new_sample without
  run_opencode "$PROMPT_L4"
  refute grep -q "$SENTINEL_L4" "$WORK/lib/calc.py"
